...
const REQUIRED_FIELDS = ["name", "email", "phone", "profile", "audience"] as const;

export function PartnerForm() {
  const tf = useT().partners.form;
  ...
  const isFieldValid = (key: keyof PartnerInput) => {
    const v = values[key];
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5" noValidate>
      ...
      <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
        ...
        <Field label={tf.profile} htmlFor="profile" error={errors.profile?.message} valid={isFieldValid("profile")}>
          <Select
            value={profile}
            onValueChange={(v) => {
              setValue("profile", v as PartnerInput["profile"], {
                shouldValidate: true,
                shouldTouch: true,
              });
            }}
          >
            ...
            >
              <SelectValue placeholder={tf.profilePh} />
            </SelectTrigger>
            <SelectContent>
              {partnerProfiles.map((p) => (
                <SelectItem key={p.value} value={p.value}>
                  {tf.profiles[p.value] ?? p.label}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
      ...
      <Field
        label={tf.link}
        htmlFor="link"
        optionalLabel={tf.optional}
        error={errors.link?.message}
        valid={
          !!touchedFields.link && !errors.link && !!values.link && values.link.length > 0
        }
      >
        <Input
          id="link"
      ...
      <Field label={tf.audience} htmlFor="audience" error={errors.audience?.message} valid={isFieldValid("audience")}>
        <Textarea
          id="audience"
          rows={5}
          placeholder={tf.audiencePh}
          ...
          {...register("audience", { onBlur: () => trigger("audience") })}
        />
      </Field>
